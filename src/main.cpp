...
#if defined(NDEBUG)
# error "CloudStorageIntegral cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CloudStorageIntegral Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // CloudStorageIntegral: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// CloudStorageIntegral: find block wanted by given orphan block
...
// CloudStorageIntegral: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal CloudStorageIntegral miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // CloudStorageIntegral: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CloudStorageIntegral: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // CloudStorageIntegral: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // CloudStorageIntegral: track money supply and mint amount info
...
// CloudStorageIntegral: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // CloudStorageIntegral: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // CloudStorageIntegral: compute stake entropy bit for stake modifier
    ...
    // CloudStorageIntegral: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // CloudStorageIntegral: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // CloudStorageIntegral: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// CloudStorageIntegral: attempt to generate suitable proof-of-stake
...
{
    RenameThread("CloudStorageIntegral-loadblk");
